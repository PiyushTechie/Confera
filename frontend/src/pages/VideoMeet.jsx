...
export default function VideoMeetComponent() {
  ...
  // 1. Create Peer (Passive - Wait for signal)
  ...
  // 2. Initiate Call (Active - Send Offer)
  ...
  const connectSocket = () => {
    ...
    // --- CRITICAL FIX: THE HANDSHAKE PROTOCOL ---

    // 1. If you are NEW, just save the list. DO NOT CALL. (Passive)
    ...
    // 2. If you are EXISTING, you get notified of a NEW user. CALL THEM. (Active)
    socketRef.current.on("user-joined", (user) => {
      setUserMap(prev => ({ ...prev, [user.socketId]: user }));
      createPeer(user.socketId); 
      initiateOffer(user.socketId); // Only the existing user calls
    });
    ...
    socketRef.current.on("signal", async (fromId, msg) => {
      ...
      if (!pc) pc = createPeer(fromId); // Create peer if we are the receiver
        ...
        <div className="flex flex-col h-screen relative">
          ...
          {viewMode === "SPOTLIGHT" && spotlightId === "local" && (
            <div className="absolute w-32 md:w-48 aspect-video bg-neutral-800 rounded-lg overflow-hidden shadow-2xl border border-neutral-700 z-50 cursor-move" style={{ left: `${position.x}px`, top: `${position.y}px` }} onMouseDown={handleMouseDown}>
               <video ref={localFloatingRef} autoPlay muted playsInline className="w-full h-full object-cover pointer-events-none" />
            </div>
          )}
