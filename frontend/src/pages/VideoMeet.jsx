...
export default function VideoMeetComponent() {
  ...
  useEffect(() => {
    ...
    // Cleanup analysers for users who left
    ...
    const interval = setInterval(() => {
        ...
        Object.entries(audioAnalysersRef.current).forEach(([socketId, analyser]) => {
            try {
                ...
                // Ignore audio processing errors to prevent crash
            }
        });
  ...
  const connectSocket = () => {
    ...
    socketRef.current.on("hand-toggled", ({ socketId, isRaised }) => {
        ...
        setUserMap(prev => ({ ...prev, [socketId]: { ...prev[socketId], isHandRaised: isRaised } }));
    });
              ...
              >
                  {isCamOff ? (
                      <div className="w-full h-full flex items-center justify-center bg-neutral-700">
                          <span className="text-xl font-bold text-gray-300">{displayName.charAt(0).toUpperCase()}</span>
                      </div>
                  ) : (
