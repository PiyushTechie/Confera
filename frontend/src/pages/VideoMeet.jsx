...
export default function VideoMeetComponent() {
  ...
  // --- CRITICAL: State for UI Rendering, Ref for Logic ---
  ...
  // Layout & Logic
  const [viewMode, setViewMode] = useState("SPEAKER"); // Default to Speaker (Zoom style)
  const [activeSpeakerId, setActiveSpeakerId] = useState(null);
  const [pinnedUserId, setPinnedUserId] = useState(null);
  const [gridPage, setGridPage] = useState(0);
  const GRID_PAGE_SIZE = 4; // Max 4 per page
  ...
  const renderMainSpotlight = () => {
      ...
      if(isLocal) { 
          ...
          stream = localStream; // Use STATE stream
          isCamOff = !video; 
  ...
  const renderSideStrip = () => {
      ...
      return stripParticipants.map(p => {
          ...
          const isThisHost = p.isLocal ? isHost : pId === roomHostId;
          ...
          <div className="bg-neutral-800 p-8 rounded-xl shadow-2xl w-full max-w-md border border-neutral-700">
            ...
            <div className="relative mb-6 rounded-lg overflow-hidden bg-black aspect-video flex items-center justify-center">
                {localStream ? (
                    ...
                    <div className="text-gray-400 flex flex-col items-center gap-2"><Loader2 size={32} className="animate-spin" /> Starting Camera...</div>
                )}
            </div>
          ...
          {showParticipants && (
            <div className="absolute right-0 top-0 h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] w-full md:w-80 bg-neutral-800 border-l border-neutral-700 z-30 flex flex-col slide-in-right">
                ...
                <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                    ...
                    <div className="flex justify-between items-center bg-neutral-700/50 p-2 rounded"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-xs">{(userName || "G").charAt(0)}</div><span className="text-sm">{userName} (You) {isHost && <Crown size={12} className="text-yellow-400 fill-yellow-400 inline ml-1" />}</span></div></div>
