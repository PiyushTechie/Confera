...
export default function VideoMeetComponent() {
  ...
  const [activeMenuId, setActiveMenuId] = useState(null); // For participant dropdown
  ...
  const connectSocket = () => {
    ...
    socketRef.current.on("user-joined", (user) => {
      ...
      initiateOffer(user.socketId, pc);
      addToast(`${user.username} joined`);
    });
    ...
    socketRef.current.on("receive-message", (data) => {
      setMessages((prev) => [
        ...prev,
        { ...data, isMe: data.socketId === socketRef.current.id },
      ]);
      if (!showChat && data.socketId !== socketRef.current.id)
    ...
    socketRef.current.on("lock-update", (isLocked) => {
      ...
      addToast(isLocked ? "Meeting Locked 🔒" : "Meeting Unlocked 🔓", "info");
    });
  ...
  const createPeer = (targetId) => {
    ...
    pc.ontrack = (e) =>
      setVideos((prev) => {
        if (prev.some((v) => v.socketId === targetId)) return prev;
        return [...prev, { socketId: targetId, stream: e.streams[0] }];
      });
    return pc;
  };
  ...
  const handleMuteAll = () => {
    if (window.confirm("Mute all?")) {
      socketRef.current.emit("mute-all");
      addToast("Muted everyone");
    }
  };
  const handleStopVideoAll = () => {
    if (window.confirm("Stop all video?")) {
      socketRef.current.emit("stop-video-all");
      addToast("Stopped videos");
    }
  };
  const handleTransferHost = (id) => {
    if (window.confirm("Transfer host rights?")) {
      socketRef.current.emit("transfer-host", id);
      addToast("Host transferred");
    }
  };
  const handleEndCall = () => {
    if (amIHost && window.confirm("End meeting for everyone?")) {
      socketRef.current.emit("end-meeting-for-all");
      setTimeout(cleanupAndLeave, 100);
    } else cleanupAndLeave();
  };
  const handleScreen = async () => {
    if (isMobile) return addToast("Not supported on mobile", "error");
    if (!screen) {
      try {
  ...
  const handleTileClick = (id) =>
    setPinnedUserId(id === pinnedUserId ? null : id);
  ...
  useEffect(() => {
    let mounted = true;
    getMedia().then(() => {
      if (mounted && (bypassLobby || (username && username !== "Guest")))
        connectSocket();
  ...
  const getMainSocketId = () =>
    pinnedUserId ||
    activeSpeakerId ||
    (videos.length > 0 ? videos[0].socketId : "local");
  ...
  const renderPaginatedGrid = () => {
    ...
    return (
      <div className="relative w-full h-full bg-black p-2 flex flex-col items-center justify-center">
        <div
          className={`grid ${gridClass} gap-2 w-full max-w-6xl h-full transition-all`}
        >
          {visible.map((p) => {
            const pId = p.isLocal ? socketIdRef.current || "local" : p.socketId;
            const user = p.isLocal
              ? { username: userName, isMuted: !audio, isVideoOff: !video }
  ...
  const renderMainSpotlight = () => {
    ...
    const user = isLocal
      ? { username: userName, isVideoOff: !video, isMuted: !audio }
      : userMap[mainId] || { username: "Guest" };
    const stream = isLocal
      ? localStream
      : videos.find((v) => v.socketId === mainId)?.stream;
  ...
  const renderSideStrip = () => {
    ...
    return all
      .filter(
        (p) =>
          (p.isLocal ? socketIdRef.current || "local" : p.socketId) !== mainId
      )
      .map((p) => {
        const pId = p.isLocal ? socketIdRef.current || "local" : p.socketId;
        const user = p.isLocal
          ? { username: userName, isVideoOff: !video }
          : userMap[pId] || { username: "Guest" };
        return (
          <div
            key={pId}
            onClick={() => setPinnedUserId(pId)}
            className="w-48 aspect-video bg-neutral-800 rounded-lg overflow-hidden flex-shrink-0 border border-neutral-700 cursor-pointer relative"
          >
            {(p.isLocal && !video) || (!p.isLocal && user.isVideoOff) ? (
              <AvatarFallback username={user.username} />
            ) : (
              <VideoPlayer
                stream={p.stream}
      ...
      <div className="fixed top-20 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            ...
            } p-4 rounded shadow-2xl flex items-center gap-3 pointer-events-auto`}
          >
            {t.msg}
          </div>
        ))}
          ...
          <div className="bg-neutral-800 p-6 rounded-2xl max-w-md w-full border border-neutral-700">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  className="w-full bg-neutral-900 border border-neutral-600 rounded p-2"
              ...
              <div className="pt-4">
                ...
                >
                  {isAudioConnected ? "Leave Audio" : "Join Audio"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-neutral-800 p-8 rounded-2xl max-w-sm w-full text-center">
            ...
            <form onSubmit={handleSubmitPasscode}>
              <input
                type="password"
                autoFocus
                className="w-full bg-neutral-900 border border-neutral-600 rounded p-3 mb-4"
                placeholder="Enter Passcode"
                value={passcodeInput}
                onChange={(e) => setPasscodeInput(e.target.value)}
              />
              {passcodeError && (
                <p className="text-red-500 text-xs mb-3">Incorrect passcode</p>
              )}
              <button
                type="submit"
      ...
      {askForUsername && !showPasscodeModal && (
        <div className="flex flex-col items-center justify-center min-h-screen p-4">
          <div className="bg-neutral-800 p-8 rounded-xl max-w-md w-full border border-neutral-700">
            ...
            <input
              className="bg-neutral-700 border border-neutral-600 text-white rounded block w-full p-2.5 mb-6"
              placeholder="Name"
              value={userName}
              onChange={(e) => setUsername(e.target.value)}
            />
            <div className="bg-black aspect-video rounded overflow-hidden mb-6 relative">
              {localStream ? (
                <VideoPlayer
                  stream={localStream}
                  isLocal={true}
            ...
            <button
              onClick={connect}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium rounded p-3"
            >
              Join
            </button>
          </div>
      ...
      {isInWaitingRoom && !askForUsername && !showPasscodeModal && (
        <div className="flex items-center justify-center min-h-screen bg-neutral-900">
          <div className="text-center">
            <ShieldAlert size={40} className="mx-auto text-yellow-500 mb-6" />
        ...
        <div className="flex flex-col h-screen relative">
          <div className="flex-1 flex flex-col md:flex-row bg-black overflow-hidden relative">
            <div className="flex-1 flex flex-col relative overflow-hidden">
              ...
              {viewMode !== "GRID" && !isMobile && (
                <div className="h-28 border-t border-neutral-800 bg-neutral-900 flex overflow-x-auto p-2 gap-2">
                  {renderSideStrip()}
                </div>
              )}
            </div>
              ...
              <div className="w-full md:w-80 bg-neutral-900 border-l border-neutral-800 flex flex-col absolute inset-0 md:static z-40 animate-in slide-in-from-right duration-200">
                ...
                {amIHost && (
                  <div className="p-4 border-b border-neutral-800 flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-neutral-800 hover:bg-neutral-700 py-2 rounded text-xs text-red-400 font-bold border border-red-900/50"
                    >
                      Mute All
                    </button>
                    <button
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  {Object.values(userMap).map((u) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-neutral-700 flex items-center justify-center font-bold">
                          {u.username.charAt(0)}
                        </div>
                          ...
                          <div className="relative">
                            ...
                            {activeMenuId === u.socketId && (
                              <div className="absolute right-0 top-6 w-32 bg-neutral-800 border border-neutral-700 rounded shadow-xl z-50 flex flex-col">
                                ...
                                <button
                                  onClick={() => {
                                    handleTransferHost(u.socketId);
                                    setActiveMenuId(null);
                                  }}
                                  className="px-3 py-2 text-left text-xs hover:bg-neutral-700 flex items-center gap-2"
                                >
                                  <UserCog size={12} /> Make Host
                                </button>
                              </div>
                            )}
                          </div>
            ...
            {showChat && (
              <div className="w-full md:w-80 bg-neutral-900 border-l border-neutral-800 flex flex-col absolute inset-0 md:static z-40 animate-in slide-in-from-right duration-200">
                    ...
                    >
                      <div
                        className={`max-w-[85%] rounded-xl p-3 ${
                          m.isMe ? "bg-blue-600" : "bg-neutral-800"
                        }`}
                      >
                        {!m.isMe && (
                ...
                <div className="p-4 border-t border-neutral-800 relative">
                  <input
                    className="w-full bg-neutral-800 rounded-full py-3 pl-4 pr-12 text-sm outline-none"
                  ...
                  <button
                    ...
                    className="absolute right-6 top-1/2 -translate-y-1/2 text-blue-500 hover:text-blue-400"
                  >
                    <Send size={18} />
                  </button>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-full bg-neutral-900 border border-neutral-600 rounded p-2 text-xs text-gray-300"
                      />
                      <button
          ...
          <div className="hidden md:flex h-20 bg-neutral-900 border-t border-neutral-800 items-center justify-center z-20 px-4 gap-4 relative">
            <button
              ...
              className="p-3 rounded-full bg-neutral-800 hover:bg-neutral-700 absolute left-4 flex flex-col items-center gap-1 group"
            >
              <Settings size={20} />
            ...
            <button
              ...
              className="p-3 rounded-xl bg-red-600 px-6 font-bold hover:bg-red-700 transition-colors text-white"
            >
              End Call
            </button>
            ...
            <div className="absolute right-4 flex gap-3">
              ...
              >
                ...
                {amIHost && waitingUsers.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-neutral-900"></span>
                )}
                <span className="text-[10px] text-gray-400 group-hover:text-white">
                  People
                </span>
              ...
              >
                ...
                {unreadMessages > 0 && (
                  <span className="absolute top-0 right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-neutral-900"></span>
                )}
                <span className="text-[10px] text-gray-400 group-hover:text-white">
                  Chat
                </span>
