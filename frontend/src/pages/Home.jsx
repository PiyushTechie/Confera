...
// --- NAVBAR COMPONENT (Internal for Home) ---
const Navbar = ({ navigate, handleLogout }) => {
    // REPLACE THIS WITH YOUR ACTUAL IMAGE PATH
    const brandLogoSrc = brandLogo;

    return (
        <nav className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 border-b border-slate-200/60 px-4 sm:px-6 lg:px-8 py-4">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-all"
                    >
                        <Clock size={18} />
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-full transition-all"
                    >
                        <LogOut size={18} />
...
function HomeComponent() {
    ...
    const [passcode, setPasscode] = useState(""); // <--- Passcode State
    ...
    return (
        <div className="min-h-screen w-full bg-slate-50 flex flex-col font-sans text-slate-800">
            ...
            {/* Main Hero Content */}
            <div className="flex-1 flex flex-col items-center justify-center w-full max-w-7xl mx-auto px-6 -mt-10">
                
                {/* Clock Section */}
                <div className="text-center mb-12 sm:mb-20">
                    <div className="inline-block px-4 py-1 mb-4 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-xs font-bold tracking-wider uppercase">
                        Secure Video Conferencing
                    </div>
                    <h1 className="text-6xl sm:text-7xl md:text-8xl font-bold text-slate-800 mb-2 tracking-tight">
                        {formatTime(date)}
                    </h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 w-full max-w-4xl">
                    ...
                    <button
                        ...
                        className="group relative flex flex-col items-center justify-center gap-4 p-6 sm:p-8 bg-orange-500 hover:bg-orange-600 rounded-3xl shadow-xl shadow-orange-500/20 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl"
                    >
                        <div className="p-4 bg-white/20 rounded-2xl group-hover:scale-110 transition-transform duration-300">
                            <Video size={32} className="text-white" />
                        </div>
                        <span className="text-lg font-bold text-white">New Meeting</span>
                    </button>

                    {/* JOIN */}
                    <button
                        onClick={() => { setMeetingCode(""); setPasscode(""); setShowJoinInputModal(true); }}
                        className="group flex flex-col items-center justify-center gap-4 p-6 sm:p-8 bg-white hover:bg-indigo-50 rounded-3xl shadow-sm border border-slate-200 transition-all duration-300 transform hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg"
                    >
                        <div className="p-4 bg-indigo-100 text-indigo-600 rounded-2xl group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
                            <Plus size={32} />
                        </div>
                        <span className="text-lg font-bold text-slate-700 group-hover:text-indigo-700">Join</span>
                    </button>

                    {/* SCHEDULE */}
                    <button className="group flex flex-col items-center justify-center gap-4 p-6 sm:p-8 bg-white hover:bg-indigo-50 rounded-3xl shadow-sm border border-slate-200 transition-all duration-300 transform hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg">
                        <div className="p-4 bg-indigo-100 text-indigo-600 rounded-2xl group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
                            <Calendar size={32} />
                        </div>
                        <span className="text-lg font-bold text-slate-700 group-hover:text-indigo-700">Schedule</span>
                    </button>
                    ...
                    <button className="group flex flex-col items-center justify-center gap-4 p-6 sm:p-8 bg-white hover:bg-indigo-50 rounded-3xl shadow-sm border border-slate-200 transition-all duration-300 transform hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg">
                        ...
                        <span className="text-lg font-bold text-slate-700 group-hover:text-indigo-700">Share Screen</span>
                    </button>
                </div>
            </div>
            ...
            {showPreviewModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-auto md:h-[600px] border border-white/10">
                        
                        {/* LEFT: Video Preview Area */}
                        <div className="md:w-2/3 bg-black relative flex flex-col justify-center p-4">
                            <div className="relative w-full h-full rounded-2xl overflow-hidden bg-neutral-900 border border-white/10 shadow-2xl">
                                <video 
                                    ref={localVideoRef} 
                                    autoPlay 
                                    muted 
                                ...
                                {!isVideoOn && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="w-24 h-24 bg-indigo-600 rounded-full flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                                            {participantName ? participantName.charAt(0).toUpperCase() : "You"}
                                        </div>
                                    </div>
                                )}

                                {/* Meeting ID Overlay */}
                                <div className="absolute top-4 left-4 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-lg flex items-center gap-2 border border-white/10">
                        ...
                        <div className="md:w-1/3 bg-white p-8 flex flex-col">
                            <div className="flex justify-between items-center mb-8">
                                <h2 className="text-2xl font-bold text-slate-800">
                                    {isJoining ? "Join Meeting" : "Ready to join?"}
                                </h2>
                                <button onClick={stopPreviewCamera} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                                    <X size={20} />
                                </button>
                            </div>
